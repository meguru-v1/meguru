...
const getDiningRule = (durationMinutes: number) => {
    if (durationMinutes <= 150) {
        return `- **食事・カフェの制限**: 各コースにおいて **最大1件** まで (厳守)。残りは必ず観光・体験スポットを選ぶこと。`;
    } else if (durationMinutes <= 300) {
        return `- **食事・カフェの制限**: 各コースにおいて **最大1件〜2件** まで (厳守)。体験・景色をメインに据えること。`;
    } else {
        return `- **食事・カフェの制限**: 各コースにおいて **最大2件** まで (厳守)。体験・景色をメインに据えること。`;
    }
};
